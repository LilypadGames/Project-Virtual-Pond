...
class PlayerData {
    ...
    async getPlayerData() {
        ...
        var pathExists = await database.pathExists('users/' + this.socket.request.user.data[0].id + '/character');

        //get character data from database
        if (pathExists) {
            playerData.character = {
                eye_type: await database.getValue('users/' + this.socket.request.user.data[0].id + '/character/eye_type'),
                color: await database.getValue('users/' + this.socket.request.user.data[0].id + '/character/color')
            };
        };

        //check if currency data exists
        pathExists = await database.pathExists('users/' + this.socket.request.user.data[0].id + '/currency');

        //get currency data from database
        if (pathExists) {
            playerData.currency = {
                clovers: await database.getValue('users/' + this.socket.request.user.data[0].id + '/currency/clovers')
            };
        };
    ...
    storePlayerData() {
       ...
       //character
        if (this.socket.player.character) {
            ...
            if (this.socket.player.character.eye_type != undefined) database.setValue(path + '/eye_type', this.socket.player.character.eye_type);
            if (this.socket.player.character.color != undefined) database.setValue(path + '/color', this.socket.player.character.color);
        };

        //currency
        if (this.socket.player.currency) {
            path = 'users/' + this.socket.player.id + '/currency'
            if (this.socket.player.currency.clovers != undefined) database.setValue(path + '/clovers', this.socket.player.currency.clovers);
        };
    };
    ...
    requestClientPlayerData() {
        ...
        //log
        console.log(utility.timestampString('PLAYER ID: ' + this.socket.player.id + ' (' + this.socket.player.name + ')' + ' - Requested Player Data: ' + this.socket.player.id + ' (' + this.socket.player.name + ')'));
    ...
    async requestAllPlayersInRoom() {
        ...
        //log
        console.log(utility.timestampString('PLAYER ID: ' + this.socket.player.id + ' (' + this.socket.player.name + ')' + ' - Reloaded Room: ' + this.socket.roomID));
    ...
    updateClientPlayerData(data) {
        ...
        //log
        console.log(utility.timestampString('PLAYER ID: ' + this.socket.player.id + ' (' + this.socket.player.name + ')' + ' - Updated Player Data'));

        //update character
        if (!this.socket.player.character && data.character) this.socket.player.character = {};
        if (data.character) {
            if (data.character.eye_type != undefined) this.socket.player.character.eye_type = data.character.eye_type;
            if (data.character.color != undefined) this.socket.player.character.color = data.character.color;
        };

        //update currency
        if (!this.socket.player.currency && data.currency) this.socket.player.currency = {};
        if (data.currency) {
    ...
    resetMessageData(messageID) {
        ...
        if (this.socket.player.message) {
            if (this.socket.player.message.id == messageID) delete this.socket.player.message;
        };
    };
};
